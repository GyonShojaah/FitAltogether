...
Although previous work on unmixing the spectra of Earth from disk-integrated multi-band light curves appeared successful, here we show that there is a mathematical degeneracy between the surface colors and their spatial  distributions. 
Nevertheless, \edit{useful constraints on the spectral shape of individual surface types may be obtained due to the constraint that albedo should be between 0 to 1, depending the trajectory of the planet in color space}.  
...
\includegraphics[width=\hsize]{schematics_ver2.pdf}
    \end{center}
    \caption{Schematic figure to illustrate the relation between surface spectra ($\{{\bf s}_k\} $; filled points) and the observed data ($\{{\bf d}_i\} $; crosses). }
\label{fig:schematic}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
we use PCA to extract two dominant principal components through PCA with other components having virtually zero contributions, consistent with the 3 input surface types.  
...
Since we use the same thre surface spectra throughout, the principal component plane is the same in all cases, but the individual PCs may be rotated. 

Using $V_{nj}$, the light curves at $\alpha = 90^{\circ }$ ($d_{ij}$) are  projected onto the PC plane by
%%%
\begin{equation}
d_{ij} = \sum_n U_{in} V_{nj} + \bar d_j
\end{equation}
%%%
where $U_{in}$ is the trajectory in the PC plane and $\bar d_j$ is the time average of the colors. % as the offset. 
...
The dominant PCs ($V_{nj}$) and the time-averaged colors ($\bar d_j$) uniquely specify the PC plane. 

We may plot the trajectories of other light curves at different phase angles on the same plane because the PC plane should be the same plane regardless of orbital phase. %(under the noiseless condition) because we use the same 3 surface types and assume Lambertian surfaces. 
...
\end{equation}
%%%
given the symmetry of ${\bf t}_1$, ${\bf t}_2$, and ${\bf t}_3$ \memoYF{Yes, I mean labeling degeneracy}. 
...
Nevertheless, we are likely to find one surface type in the upper part of the allowed region, near label A. 
...
As a result, imposing Gaussian Process tends to yield better estimation of ocean color and biased estimation of \edit{sand} color. 
...
In this paper, we revisited the problem of estimating the albedo spectra of major surface type and their distribution from disk-integrated multi-band photometry of exoplanets. 
We pointed out the inherit degeneracy which makes it impossible to find unique solutions. 
Despite the degeneracy, some useful constraints on the surface spectra can be obtained due to the nature of albedo which is between 0 and 1. 
We demonstrate using a simplified toy model of Earth that such constraints let us pin down the likely solution for the surface types and their geographical maps. 
...
To illustrate these two effects, we created two additional mock light curves, (a) with the geographical maps of the \edit{sand} and vegetation being swapped, and  (b) with the geographical maps of the \edit{sand} and ocean being swapped. 
The left and right panels of Figure \ref{fig:swap} correspond to (a) and (b), respectively. 
%
...
%
Meanwhile, in (b) where we swapped the \edit{sand} and ocean, although \edit{sand} is the dominant component, the true projected color of the \edit{sand} is poorly constrained due to the large available parameter space in the upper region of the PC plane. 
...
%    \end{center}
     \end{minipage}
    \end{center}
    \caption{Left: Trajectory of the light curves with the geographical maps of the \edit{sand} and vegetation being swapped, and the resultant color contour of the posterior probability of albedo spectra of surface types. (b) Same as (a) but with the geographical maps of the \edit{sand} and ocean being swapped. \memoNBC{Show the corresponding maps!} }
\label{fig:swap}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
