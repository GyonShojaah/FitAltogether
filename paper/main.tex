...
Nevertheless, due to the constraint that albedo should be between 0 to 1, useful constraints on the spectral albedo of individual surface types may be obtained on the spectral shape of the surface types, depending the longitudinal geography of surfaces, which dictates the trajectory of the planet in color space.  
We demonstrate the degeneracy and the possible constraints using the mock data based on both a toy model of Earth and the observed data of Earth \memoYF{need ``the'' here?}. 
...
They applied their method to the light curves of the Earth observed EPOXI, and the result appeared successful in so far as they obtained reflectance spectra roughly matching the average spectra of clouds, ocean, and continents. 
...
In Section \ref{s:mockdata}, we introduce our toy-model multi-band diurnal light curves of Earth, which will be analyzed in Section \ref{s:frame}, together with the description of general framework to approach this problem. 
...
Note that in reality they are not Lambertian scatterers; among others,  scattering by the ocean is particularly anisotropic at crescent phase (Section \ref{ss:deviateLambert}).  
...
One can recast equation (\ref{eq:tilde_d_f_ast_s}) in terms of the longitudinal map:
%%%
\begin{equation}
...
Meanwhile, in (b) we swap the soil and ocean.  
Although soil is now the dominant component, 
