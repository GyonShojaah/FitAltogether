...
\includegraphics[width=\hsize]{IGBP_PCplane_Nside0.pdf}
    \end{center}
     \end{minipage}
    \caption{Making the map coarser from the upper panel of Figure \ref{fig:mockdata}. Pixelating the surface into 192 (left), 48 (middle), and 12 (right) equal-area pixels using HealPix, we plot the synthesized pixel colors with crosses on the same PC plane as Figures \ref{fig:trajectory} to \ref{fig:swap}.  The triangular shape of the locus of points becomes apparent with as few as 48 pixels. Thus, resolving the surface colors in 2 dimensions eases the challenge to identify surface types. }
